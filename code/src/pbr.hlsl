...
struct MaterialIndexConst {
    uint material_index;
};

struct SRVIndexConst {
    uint material_buffer_index;
};

ConstantBuffer<SceneConst> scene_const : register(b0);
ConstantBuffer<DrawConst> draw_const : register(b1);
ConstantBuffer<MaterialIndexConst> material_index_const : register(b2);
ConstantBuffer<SRVIndexConst> srv_indices : register(b3);
...
void psMeshPbr(
    ...
    StructuredBuffer<Material> material_buffer = ResourceDescriptorHeap[srv_indices.material_buffer_index];
    Material material = material_buffer[material_index_const.material_index];

    // Texture2D srv_ao_texture = ResourceDescriptorHeap[material.ao_tex_index];
    Texture2D srv_base_color_texture = ResourceDescriptorHeap[material.base_color_tex_index];
    ...
    const float ao = 1; // srv_ao_texture.Sample(sam_aniso, texcoords0).r;
